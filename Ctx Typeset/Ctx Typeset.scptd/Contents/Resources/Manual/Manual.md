...
: Moves all PDF files that were generated by ConTeXt to the trash. It trashes 
  only PDFs that were typeset by ConTeXt, i.e. PDFs that have a corresponding
  tex file. E.g. if you have renamed a PDF to \<original name\>-final.tex it
  will not be removed. Also any other PDF files in the folder will not be
  removed. You can combine this command with either *Purge* or *Purge all*.
